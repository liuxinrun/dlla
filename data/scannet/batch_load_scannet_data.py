...
def export_one_scan(scan_name,
    ...
    mesh_vertices, semantic_labels, instance_labels, unaligned_bboxes, \
        aligned_bboxes, instance2semantic, axis_align_matrix = export(
            mesh_file, agg_file, seg_file, meta_file, label_map_file, label_to, None,
            test_mode)
    ...
    for scan_name in scan_names:
        ...
        try:
            export_one_scan(scan_name, output_filename_prefix, max_num_point,
                            label_map_file, scannet_dir, label_to, test_mode)
        except Exception:
...
def main():
    ...
    parser.add_argument(
        '--dataset_dir',
        default='/data/lxr/dataset/scannet/',
        help='folder of the dataset.')
    parser.add_argument(
        '--label_to',
        default='nyu40id',
        help='label map type:nyu40id raw_category etc')
    parser.add_argument(
        '--output_folder',
    ...
    if args.dataset_dir is not None:
        args.output_folder = osp.join(args.dataset_dir, args.output_folder)
        args.train_scan_names_file = osp.join(args.dataset_dir, args.train_scan_names_file)
        args.label_map_file = osp.join(args.dataset_dir, args.label_map_file)
        args.train_scannet_dir = osp.join(args.dataset_dir, args.train_scannet_dir)
        args.test_scannet_dir = osp.join(args.dataset_dir, args.test_scannet_dir)
        args.test_scan_names_file = osp.join(args.dataset_dir, args.test_scan_names_file)
    batch_export(
        args.max_num_point,
        args.output_folder,
