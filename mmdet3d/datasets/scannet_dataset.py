...
from mmdet3d.utils.misc import nms_3d_faster_samecls
...
class ScanNetDataset(Custom3DDataset):
    ...
    def get_ann_info(self, index):
        ...
        if info['annos']['gt_num'] != 0:
            ...
            gt_xyzxyz = np.concatenate((gt_bboxes_3d[:, :3] - gt_bboxes_3d[:, 3:]/2, gt_bboxes_3d[:, :3] + gt_bboxes_3d[:, 3:]/2), -1)
            class_bboxes = np.concatenate((gt_xyzxyz, np.expand_dims(np.ones_like(gt_labels_3d),axis=1), np.expand_dims(gt_labels_3d,axis=1)), 1)
            # class_scores = np.ones_like(gt_labels_3d)
            nms_ids = nms_3d_faster_samecls(class_bboxes, 0.4)
            gt_bboxes_3d = gt_bboxes_3d[nms_ids]
            gt_labels_3d = gt_labels_3d[nms_ids]
        else:
