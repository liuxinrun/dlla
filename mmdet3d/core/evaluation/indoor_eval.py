...
def eval_det_cls(pred, gt, iou_thr=None):
    ...
    for iou_idx, thresh in enumerate(iou_thr):
        ...
        ret.append((npos, num_dets, recall, precision, ap))
...
def eval_map_recall(pred, gt, ovthresh=None):
    ...
    for label in gt.keys():
        for iou_idx, thresh in enumerate(ovthresh):
            if label in pred:
                num_gts[label], num_dets[label], recall[iou_idx][label], precision[iou_idx][label], ap[iou_idx][
                    label] = ret_values[label][iou_idx]
            else:
                num_gts[label], num_dets[label] = np.zeros(1), np.zeros(1)
                recall[iou_idx][label] = np.zeros(1)
                precision[iou_idx][label] = np.zeros(1)
                ap[iou_idx][label] = np.zeros(1)

    return num_gts, num_dets, recall, precision, ap
...
def indoor_eval(gt_annos,
    ...
    num_gts, num_dets, rec, prec, ap = eval_map_recall(pred, gt, metric)
    ret_dict = dict()
    header = ['classes']
    table_columns = [[label2cat[label]
                      for label in ap[0].keys()] + ['Overall']]
    
    header.append("num_gts")
    table_columns.append(list(map(int, list(num_gts.values())+[np.sum(list(num_gts.values()))])))
    header.append("num_dets")
    table_columns.append(list(map(int, list(num_dets.values())+[np.sum(list(num_dets.values()))])))
