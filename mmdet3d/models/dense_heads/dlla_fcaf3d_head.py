...
from mmdet3d.core.visualizer.show_result import box_to_corners
...
class DLLA_fcaf3d_Head(BaseModule):
    ...
    def _init_layers(self, in_channels, out_channels, n_reg_outs, n_classes):
        ...
        self.qkv = nn.Linear((n_classes+6)*3, self.channel*3)
        ...
        self.k_linear = nn.Linear((n_classes+6), self.channel)
    ...
    def _loss_single(self,
        ...
        if self.mode == "learn":
            if self.iter_test > 0:
                assigned_ids, center_targets, loss_weight, l2_loss_inter, l2_loss_intra_axis_align, dn_loss = self._get_targets(points, bbox_preds, cls_preds, localization_preds, gt_bboxes, gt_labels, img_meta)
            else:
                with torch.no_grad():
    ...
    def _nms(self, bboxes, scores, img_meta):
        ...
        if self.test_cfg.class_agnostic:
    ...
    def _get_targets(self, points, bbox_preds, cls_preds, localization_preds,  gt_bboxes, gt_labels, img_meta):
        ...
        if self.iter_test < 1:
            ...
            if (min_3k_inds >= 0).sum() == 0:
                return min_3k_inds,  gt_labels.new_full((n_points,), -1), gt_labels.new_full((n_points,), -1), None, None, None
        ...
        return min_inds, center_targets, cost_weight_raw, l2_loss_inter, l2_loss_intra_axis_align, dn_loss
